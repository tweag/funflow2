# Stolen from hasktorch, which in turn got it from plutus 
#   https://github.com/hasktorch/hasktorch/blob/master/nix/haddock-combine.nix
#   https://github.com/input-output-hk/plutus/blob/master/nix/haddock-combine.nix
#
# This function takes the output docs from a haskell.nix package library with a doc attribute
# and combines them into a single derivation with relative links in the html files instead of
# absolute links. This allows them to be deployed places like GitHub pages as a part of CI builds.
{ runCommand, lib, ghc }:
